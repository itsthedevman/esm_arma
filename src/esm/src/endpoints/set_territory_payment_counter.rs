...
pub fn set_territory_payment_counter(
    ...
    if !BOOTED.load(Ordering::SeqCst) {
        error!("[set_territory_payment_counter] ❌ Extension endpoint called before successful initialization. Ensure the Arma server properly loaded @esm and check the extension logs for initialization errors");
        return Ok(());
    }
