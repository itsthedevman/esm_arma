...
try
{
	...
	_serverTime = time;
	if (getNumber(missionConfigFile >> "CfgGrinding" >> "notifyServer") isEqualTo 1) then
	{
		_cooldown = getNumber(missionConfigFile >> "CfgGrinding" >> "notificationCooldown") * 60;
		if (_serverTime > ((_territory getVariable ["ExileGrindingNotifiedTime", (_cooldown * -1)]) + _cooldown)) then 
		{
			["baguetteRequest", ["Warning! Lock grinding in progress!"]] call ExileServer_system_network_send_broadcast;
			_territory setVariable ["ExileGrindingNotifiedTime", _serverTime];
		};
	};
	if (_serverTime > ((_territory getVariable ["ExileXM8MobileNotifiedTime",-1800]) + 1800)) then
	{
		_territory call ExileServer_system_xm8_sendGrindingStarted;
